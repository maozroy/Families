...
export const ADVANCED_EDITABLE = [
  'branch', 'generation', 'relation_to_root', 'source', 'no_sync', 'deceased',
  'dna_23andme', 'lang', 'birth_precision', 'wiki_title_he', 'wiki_title_en', 'relation_he',
];
...
const ADDED_COLUMNS = [
  ...
  /* The Hebrew half of `relation_to_root`, which was English-only — so the
     Hebrew wiki page and any Hebrew reader got a kinship term in the wrong
     language or none at all. Written by scripts/relabel-kinship.mjs beside the
     English one, never independently: two columns that can disagree about the
     same fact is the bug this whole field already has once. */
  ['relation_he', "TEXT NOT NULL DEFAULT ''"],
...
export function publicPerson(db, p, { owner = false, addressAt = null, docCount = null, wiki = null } = {}) {
  const rec = {
    ...
    facebook: p.facebook || '',
    linkedin: p.linkedin || '',
    // `relation_to_root` is deliberately NOT here. It is research bookkeeping
    // written from one person's point of view and in English ("mother's sister
    // (aunt)"); both pages compute the real relation for whoever is reading, so
    // shipping it only gave every relative a second, wronger answer. The column
    // stays — family.mjs builds the wiki from it.
